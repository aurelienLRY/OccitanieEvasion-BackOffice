    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      >
        ...
        <div className="flex flex-col justify-center items-center gap-1">
          <h2 className="text-2xl font-bold">{sessionData ? "Modifier la session" : "Créer une session"}</h2>
          {err && <div className="text-red-500 text-sm">{err}</div>}
{result && result.success && <div className="text-green-500 text-sm">Session créée avec succès</div>}
        </div>
        ...
        <div className="flex flex-col items-center gap-1 border-2 rounded-md border-sky-500 w-full p-2">
          ...
          <div className="flex flex-col md:flex-row items-center gap-2">
            ...
            {/* HEURE DE DÉBUT */}
            <Input name="startTime" type="time" label="Heure de début" className=" " />
            {/* HEURE DE FIN */}
            <Input name="endTime" type="time" label="Heure de fin" className="" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-2">
          {/* ACTIVITÉ */}
          <SelectInput name="activity" options={activities.map(activity => ({ id: activity._id, name: activity.name }))} label="Activité" className="" />
          {/* LIEU */}
          <SelectInput name="spot" options={spots.map(spot => ({ id: spot._id, name: spot.name }))} label="Lieu" className="" />
        </div>
        </div>

        <div className="flex flex-col md:flex-row  gap-3 items-center md:justify-evenly w-full">
            <p className="text-sky-500 text-xl font-bold">Gestion des places</p>
            <div className="flex flex-col md:flex-row gap-2">
                <Input name="placesMax" type="number" label="Places max" className="" />
                <Input name="placesReserved" type="number" label="Places réservées" className="" />
            </div>
        ...
        <div className="flex justify-end items-center gap-1">
          <button
            type="submit"
            className="p-1 px-2 rounded-md bg-sky-500 text-white"
          >
