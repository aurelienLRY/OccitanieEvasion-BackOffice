...
import { ICallback} from '../types/callback.action.type'
 ...
 */
export default function ToasterAction({result , defaultMessage}: {result: ICallback , defaultMessage: string}) {
 if (result.success){
    toast.success(result.feedback && result.feedback.map((feedback:string)=> <p key={feedback}>{feedback}</p>)   || defaultMessage)
 }else{
    toast.error(
