import { ICustomerSession , ISessionWithDetails } from "@/types";
   ...
   * @returns : number
   */
  export const calculateNumberOfSessions = (sessions: ISessionWithDetails[], type: "month" | "year") => {
    const now = new Date();
 ...
 * @returns : number
 */
export const calculateInscrit = (sessions: ISessionWithDetails[], type: "month" | "year") => {
  const now = new Date();
let total = 0;
  if(type === "month") {
    const month = now.getMonth();
 ...
 * @returns : number
 */
export const getSessionByStatus = (sessions: ISessionWithDetails[], status: "Pending" | "Validated" | "Archived") => {
